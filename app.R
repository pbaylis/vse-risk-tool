...
ui <- fluidPage(
  titlePanel(title = div(
    ...
    h2("Purpose"),
    helpText("The VSE COVID Risk/Reward Assessment Tool (VSE COVID Risk Tool, hereafter) can be used to compare some of the risks and benefits of re-opening different sectors of the Canadian economy. It provides an interactive figure for the entire country and for each province, where risk is represented on the vertical axis and benefit on the horizontal axis. Each sector of the economy is listed at the right of the figure and identified by a different color. Distinct occupations within a sector are identified by bubbles, where larger bubbles represent occupations that employ a greater share of a sector’s labour force."),
    ...
    h2("Instructions"),
    helpText("1. Choose either all of Canada or some province(s) using the dropdown below. Due to data privacy requirements, only national provincial data is available and data for some provinces are combined. Selecting Quebec will load a Quebec-specific site built by collaborators at UQAM, also available ", tags$a(href="https://cirano.qc.ca/fr/shiny/connollm/tool", "here.", target = "_blank")),
